...
def plotpy_item_to_annotation(item: Any) -> GraphicalAnnotation | None:
    ...
    if isinstance(item, AnnotatedSegment):
        ...
        return SegmentAnnotation(x0=x0, y0=y0, x1=x1, y1=y1, **common)
    if isinstance(item, AnnotatedObliqueRectangle):
    ...
    if isinstance(item, AnnotatedCircle):
    ...
    if isinstance(item, AnnotatedEllipse):
        x0, y0, x1, y1 = item.get_xdiameter()
    ...
    if isinstance(item, Marker):
        ...
        return CursorAnnotation(orientation=orientation, position=position, **common)
    return None
