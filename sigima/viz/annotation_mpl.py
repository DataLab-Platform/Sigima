...
def _label_location(ax, annotation: GraphicalAnnotation):
    ...
    if isinstance(annotation, PointAnnotation):
        location = (annotation.x, annotation.y, ax.transData)
        ...
        location = (annotation.cx, annotation.cy, ax.transData)
        ...
        if annotation.orientation == CursorOrientation.CROSSHAIR:
            ...
            location = (*annotation.position, ax.transData)
        else:
            assert isinstance(annotation.position, float)
            if annotation.orientation == CursorOrientation.VERTICAL:
                location = (annotation.position, 1.0, ax.get_xaxis_transform())
            else:
                location = (1.0, annotation.position, ax.get_yaxis_transform())
    elif isinstance(annotation, RangeAnnotation):
        center = (annotation.start + annotation.end) / 2
        if annotation.axis == Axis.X:
            location = (center, 1.0, ax.get_xaxis_transform())
        else:
            location = (1.0, center, ax.get_yaxis_transform())
    else:
        location = None
    return location
