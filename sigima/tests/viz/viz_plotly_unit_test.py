...
def test_curve_and_image_specs_materialize_as_plotly_figures() -> None:
    ...
    curve_figure = figure_from_spec(build_curve_figure_spec(signal))
    image_figure = figure_from_spec(build_image_figure_spec(image, results=result))
...
def test_view_curves_uses_plotly_show(monkeypatch) -> None:
    ...
    from sigima.viz import viz_plotly
