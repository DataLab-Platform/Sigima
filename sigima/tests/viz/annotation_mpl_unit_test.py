...
def test_all_annotation_primitives_create_expected_artists() -> None:
    ...
    assert sum(type(artist) is Rectangle for artist in artists) == 2
    assert sum(type(artist) is Circle for artist in artists) == 1
    assert sum(type(artist) is Ellipse for artist in artists) == 1
    assert sum(type(artist) is Polygon for artist in artists) == 1
...
def test_axes_text_uses_normalized_transform() -> None:
    ...
    [artist] = add_annotations_to_axes(axes, [annotation])

    assert artist.get_transform() != axes.transData
    plt.close(figure)
